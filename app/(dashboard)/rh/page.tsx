import { Users, ClipboardList, CreditCard, GraduationCap } from "lucide-react";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";

const sousModules = [
    { label: "Agents & Organigramme", icon: Users, phase: "Phase 3.1" },
    { label: "Congés & Absences", icon: ClipboardList, phase: "Phase 3.2" },
    { label: "Bulletins de paie", icon: CreditCard, phase: "Phase 3.2" },
    { label: "Formations & Certifications", icon: GraduationCap, phase: "Phase 3.2" },
];

export default function RHPage() {
    return (
        <div>
            <div className="mb-6">
                <h1 className="flex items-center gap-2 text-2xl font-bold">
                    <Users className="h-6 w-6 text-blue-600" />
                    Ressources Humaines
                </h1>
                <p className="mt-1 text-sm text-muted-foreground">
                    Gestion des carrières, paie, congés, formations et portail self-service
                </p>
            </div>
            <div className="grid gap-4 sm:grid-cols-2">
                {sousModules.map((sm) => (
                    <Card key={sm.label} className="border-dashed">
                        <CardHeader className="pb-2">
                            <CardTitle className="flex items-center justify-between text-base">
                                <span className="flex items-center gap-2">
                                    <sm.icon className="h-4 w-4 text-muted-foreground" />
                                    {sm.label}
                                </span>
                                <Badge variant="outline" className="text-xs">{sm.phase}</Badge>
                            </CardTitle>
                        </CardHeader>
                        <CardContent>
                            <p className="text-sm text-muted-foreground">
                                Ce sous-module sera implémenté en {sm.phase}.
                            </p>
                        </CardContent>
                    </Card>
                ))}
            </div>
        </div>
    );
}
