...
import { Search, ShieldAlert, RadioReceiver, Eye, AlertTriangle, BugOff, Flame, FilterX } from "lucide-react";
import { Button } from "@/components/ui/button";
import Link from "next/link";
import { format } from "date-fns";
import { fr } from "date-fns/locale";

export default function LimsAlertesListePage() {
    const { user } = useAuth();
    const [searchTerm, setSearchTerm] = useState("");
    const [filtreType, setFiltreType] = useState("tous");
    const [filtreNiveau, setFiltreNiveau] = useState("tous");
    const [filtreStatut, setFiltreStatut] = useState("actives"); // "actives", "toutes", "archivees"

    const alertes = useQuery(api.alertes.alertes.listerAlertes as any, { userId: user?._id as any });

    if (alertes === undefined) return <div className="p-8 text-center animate-pulse">Recherche des dossiers en cours...</div>;

    const filtered = alertes.filter((a: any) => {
        ...
        let st = true;
        if (filtreStatut === "actives") st = ["nouvelle", "en_verification", "confirmee"].includes(a.statut);
        else if (filtreStatut === "archivees") st = ["archivee", "resolue"].includes(a.statut);
    ...
    const getStatutBadge = (statut: string) => {
        switch (statut) {
            ...
            case 'resolue': return <Badge variant="outline" className="text-emerald-600 border-emerald-200 bg-emerald-50">✔️ Résolue</Badge>;
            case 'archivee': return <Badge variant="outline" className="text-slate-500 border-slate-200 bg-slate-50">📁 Archivée</Badge>;
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row justify-between md:items-end gap-4">
                <div>
                    <h1 className="text-3xl font-bold tracking-tight text-slate-900 border-l-4 border-red-600 pl-3">Registre Central des Alertes</h1>
                    <p className="text-muted-foreground mt-1 text-sm">Gestion du cycle de vie des signalements et investigations sanitaires.</p>
                </div>
                <Button className="bg-red-600 hover:bg-red-700 text-white" asChild>
                    <Link href="/alertes/alertes/nouveau">Nouvelle Alerte Manuelle</Link>
                </Button>
            </div>
            ...
            <div className="flex flex-col md:flex-row gap-4 bg-white p-4 rounded-lg shadow-sm border border-slate-200">
                <div className="flex-1 relative">
                    ...
                    <Input
                        placeholder="Rechercher un dossier, un produit..."
                ...
                <div className="flex flex-wrap md:flex-nowrap gap-2">
                    ...
                    <Select value={filtreStatut} onValueChange={setFiltreStatut}>
                        ...
                        <SelectContent>
                            <SelectItem value="toutes">Tout l'historique</SelectItem>
                            <SelectItem value="actives">En cours (Actives)</SelectItem>
                            <SelectItem value="archivees">Clôturées</SelectItem>
                        </SelectContent>
                    </Select>

                    <Button variant="outline" size="icon" onClick={() => { setSearchTerm(''); setFiltreType('tous'); setFiltreNiveau('tous'); setFiltreStatut('actives'); }}>
                                    ...
                                    <TableCell className="text-right">
                                        <div className="flex flex-col items-end gap-2">
                                            ...
                                            <Button variant="ghost" size="sm" className="h-7 text-xs flex gap-1" asChild>
                                                <Link href={`/alertes/alertes/${alerte._id}`}><Eye className="w-3 h-3" /> Ouvrir</Link>
                                            </Button>
                                        </div>
                                    </TableCell>
                                </TableRow>
            ...
            <div className="text-right text-xs text-slate-400">
                {filtered.length} alertes extraites
            </div>
        </div>
    );
}
